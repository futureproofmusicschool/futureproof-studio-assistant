    ...
    def init_api(self):
        ...
        def track_duplicate_clip_to_arrangement(track, params: Tuple[Any]):
            ...
            if clip_slot.clip is None:
                raise RuntimeError("Clip slot %d is empty" % int(clip_index))
            track.duplicate_clip_to_arrangement(clip_slot.clip, float(destination_time))
            return float(destination_time),
