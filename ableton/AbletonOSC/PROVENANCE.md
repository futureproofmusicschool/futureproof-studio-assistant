...
3. **This repo's additions:**
   - `/studio/capabilities` and `/studio/query` (osc_server.py): optional request IDs for registered read handlers. Replies echo the request ID so concurrent and late replies cannot be confused. Existing OSC addresses remain supported.
